...
* `GET /` discovery: 200, name + version + 23-name validator list.
* `GET /validators`: 200, 23 entries.
...
def test_root_returns_200_with_name_version_and_23_validators(client: TestClient) -> None:
    ...
    assert len(body["validators"]) == 23
    # The 23 names match the canonical _port._VALIDATORS list (defensive
...
def test_openapi_json_returns_200_with_all_23_validators_as_paths(client: TestClient) -> None:
    ...
    assert len(paths) >= 23, f"expected >= 23 paths, got {len(paths)}"
