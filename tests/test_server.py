...
* `GET /` discovery: 200, name + version + 41-name validator list.
* `GET /validators`: 200, 41 entries.
...
def test_root_returns_200_with_name_version_and_23_validators(client: TestClient) -> None:
    ...
    # The 41 names match the canonical _port._VALIDATORS list (defensive
...
def test_openapi_json_returns_200_with_all_23_validators_as_paths(client: TestClient) -> None:
    ...
    assert len(paths) >= 41, f"expected >= 41 paths, got {len(paths)}"
