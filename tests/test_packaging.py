...
class TestPackageRuntime:
    ...
    def test_validator_functions_are_top_level_attributes(self, installed_venv) -> None:
        """All 23 public validators should be importable as attributes.
        ...
        assert payload["n_kinds"] == 23, (
            f"expected 23 kinds from list_kinds(), got {payload['n_kinds']}"
        )
    ...
    def test_a1_validate_list_subcommand(self, installed_venv) -> None:
        """``a1-validate list`` must print 23 validators.
        ...
        assert len(entries) == 23, (
            f"expected 23 validators in `list` output, got {len(entries)}: "
            f"{entries}"
        )
