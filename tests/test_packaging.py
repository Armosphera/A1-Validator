...
class TestPackageRuntime:
    ...
    def test_validator_functions_are_top_level_attributes(self, installed_venv) -> None:
        """All 33 public validators should be importable as attributes.
        ...
        assert payload["n_kinds"] == 33, (
            f"expected 33 kinds from list_kinds(), got {payload['n_kinds']}"
        )
    ...
    def test_a1_validate_list_subcommand(self, installed_venv) -> None:
        """``a1-validate list`` must print 33 validators.
        ...
        assert len(entries) == 33, (
            f"expected 33 validators in `list` output, got {len(entries)}: "
            f"{entries}"
        )
