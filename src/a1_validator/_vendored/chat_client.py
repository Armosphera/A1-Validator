...
"""Vendored from autoresearch-sboss@6c9a9149f1dc8b7a5430d542de19f564a078418c — examples/chat-client/workflow.py.
...
def validate(input: dict) -> dict:
    ...
    result: dict = {
        ...
        # Snapshot placeholders — the real values are populated by the
        # safe_fetch mock INSIDE the call below. We re-fill them after the
        # call so the dict matches the upstream eval contract (which checks
        # last_request.{method,headers,body} on the post-call state).
        "last_request": {
            "url": None,
            "method": None,
            "headers": None,
    ...
    def _snapshot_last_request() -> None:
        """Populate result['last_request'] from the post-call call_log state."""
        options = call_log["options"] or {}
